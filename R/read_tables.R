...
read_table10 <- function(filepath, filename) {
  startingdir <- getwd()
  setwd(filepath)
  ...
  # Table 10 --------------------------------------------------------------
  #Person and Item Sep
  table10_data <- brio::read_lines(paste0(filename, "_Table_10.txt"))[4]
  pattern <- "-?\\d+(?:\\.\\d+)?|\\.\\d+"
  fit_stats <- as.numeric(unlist(regmatches(table10_data,
                                            gregexpr(pattern, table10_data))))

  #Table 10 as df
  table10_data <- brio::read_lines(paste0(filename, "_Table_10.txt"))
  ...
  mean_infit <- round(mean(result_df$`Infit MNSQ`), 2)
  mean_outfit <- round(mean(result_df$`Outfit MNSQ`), 2)
  #high_infit <- sum(result_df$`Infit MNSQ` > 1.3)
  high_outfit <- sum(result_df$`Outfit MNSQ` > 1.3)
  min_infit <- min(result_df$`Infit MNSQ`)
  max_infit <- max(result_df$`Infit MNSQ`)
  min_outfit <- min(result_df$`Outfit MNSQ`)
  max_outfit <- max(result_df$`Outfit MNSQ`)

  # Extract the variable name without the .txt extension
  df_name <- sub("\\.txt$", "", tools::file_path_sans_ext(filename))

  # Assign the table10_data frame to a variable in the global environment
  assign(df_name, result_df, envir = .GlobalEnv)

  # Table 3 --------------------------------------------------------------
  table3_data <- brio::read_lines(paste0(filename, "_Table_3.txt"))
  ...
  # Table 23 --------------------------------------------------------------
  table23_data <- brio::read_lines(paste0(filename, "_Table_23.txt"))[8:12]
  ...
  psy_line <- c(filename, fit_stats, alpha, mean_infit, min_infit,
...
read_table10 <- function(file_paths_list) {
  startingdir <- getwd()
  ...
  for (filename in file_paths_list) {
    ...
    # Ensure the data frames have unique names based on the file name
...
# Example usage with the 'testing' object you provided
testing <- list(
  c("C:/Users/jsnowden/Desktop/Other_R/HGL2_Dec23/Rasch/7_Cultural_Essentialism", "cul_ess_tp1"),
  c("C:/Users/jsnowden/Desktop/Other_R/HGL2_Dec23/Rasch/8_Racial_Essentialism", "racial_essen_tp0")
)

read_table10(testing)
...
read_table10 <- function(filename) {
  startingdir <- getwd()
...
test <- c("C:/Users/jsnowden/Desktop/Other_R/HGL2_Dec23/Rasch/7_Cultural_Essentialism", "cul_ess_tp1")
test2 <- c("C:/Users/jsnowden/Desktop/Other_R/HGL2_Dec23/Rasch/8_Racial_Essentialism", "racial_essen_tp0")

testing <- list(test, test2)
psychometrics_table(testing)
...
setwd("C:/Users/jsnowden/Desktop/Other_R/HGL2_Dec23/Rasch/7_Cultural_Essentialism")
read_table10("cul_ess_tp1")


average_infit_mnsq <- mean(cul_ess_tp1_Table_10$`Infit MNSQ`)



read_table3 <- function(filename) {

  data <- brio::read_lines(filename)
  line <- grep("\"TEST\" RELIABILITY =", data)
  extracted_value <- as.numeric(str_extract(data[line], '(?<=RELIABILITY = ).{0,4}'))
  print(extracted_value)
}

data <- brio::read_lines("cul_ess_tp1_Table_3.txt")

alpha <- read_table3("cul_ess_tp0_Table_3.txt")



#Table 10 Person and Item Rel and Sep
data <- brio::read_lines("cul_ess_tp1_Table_10.txt")[4]
pattern <- "-?\\d+(?:\\.\\d+)?|\\.\\d+"
values <- as.numeric(unlist(regmatches(data, gregexpr(pattern, data))))


#Table 23
data <- brio::read_lines("cul_ess_tp1_Table_23.txt")[8:12]
# Extract percentages using regular expressions
percentages <- regmatches(data, gregexpr("[0-9]+\\.[0-9]+%", data))
percentages <- unlist(percentages)[c(2, 4)]  # Select the desired percentages

# Extract number using string splitting
number <- as.numeric(unlist(regmatches(data[5], gregexpr("[0-9]+\\.[0-9]+", data[5]))))

# Print the extracted values
print(percentages)  # Output: "30.0%" "20.3%"
print(number)  # Output: "2.5521"


test_psychometrics <- c(values, alpha)
